...
public final class Utilities {
     ...
     *         Whether or not to account for the scroll of the descendant: sometimes this is relevant as in a child's coordinates within the
     *         descendant.
     * @return The factor by which this descendant is scaled relative to this DragLayer. Caution this scale factor is assumed to be equal in X and Y,
     * and so if at any point this assumption fails, we will need to return a pair of scale factors.
     */
    ...
    /**
     * Utility method to determine whether the given point, in local coordinates, is inside the view, where the area of the view is expanded by the
     * slop factor. This method is called while processing touch-move events to determine if the event is still within the view.
     */
    ...
    /**
     * Returns a widget with category {@link AppWidgetProviderInfo#WIDGET_CATEGORY_SEARCHBOX} provided by the same package which is set to be global
     * search activity. If widgetCategory is not supported, or no such widget is found, returns the first widget provided by the package.
     */
